        ...
        function buildCalendarCell(dateObj, items, dateStr, status) {
          ...
          const hasMorning = items.some(i => i.session === "上半");
            ...
            if (index < visibleCount) {
                ...
                const $expandedRow = $('<div class="calendar-row expanded-row"></div>');
                const $expandedCell = $('<div class="expanded-cell"></div>');

                const $header = $(`
                    <div class="expanded-cell-header">
                    <div class="d-flex justify-content-end align-items-center expanded-cell-header-top ${status === "started" ? "" : "opacity-0 pe-none"}">
